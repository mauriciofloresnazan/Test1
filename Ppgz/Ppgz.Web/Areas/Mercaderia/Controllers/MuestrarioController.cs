...
namespace Ppgz.Web.Areas.Mercaderia.Controllers
{
    [Authorize]
    [TerminosCondiciones]
    public class MuestrarioController : Controller
    {
        ...
        public ActionResult Estatus(string ProveedorNombre, int proveedorId, string NumeroProveedor, string sociedad, string fechaDesde = null, string fechaHasta = null)
        {
            var cuenta = _commonManager.GetCuentaUsuarioAutenticado();
            ...
            var pr = string.Format("{0}{1}{2}{3}", proveedor.Nombre1, proveedor.Nombre2, proveedor.Nombre3, proveedor.Nombre4);
...
WHERE id = id_muestras and NombreProveedor='" + pr + "' and Estatus in(30,40) and FechaCarga >='" + fechaf + "' and  FechaCarga <= '" + fechat + "' ");
            }
            else
            {
...
Estatus,canalm,id_d      
FROM muestras, canalmuestras       
WHERE id = id_muestras and Estatus in (20,30) and nombreproveedor='" + pr+"'");
            }
...
Estatus,canalm,id_d       
FROM muestras, canalmuestras       
WHERE id = id_muestras and NombreProveedor='" + pr + "' and Estatus = 40 and FechaCarga >='" + fechaf + "' and  FechaCarga <= '" + fechat + "' ");
            }
            else
            {
...
Estatus,canalm,id_d      
FROM muestras, canalmuestras       
WHERE id = id_muestras and NombreProveedor='" + pr + "' and Estatus=40" );
            }
            SociedadActiva = sociedad;
            ViewBag.Proveedor = proveedor;
            return View();
...
Estatus,canalm,id_d    
FROM muestras, canalmuestras       
WHERE id = id_muestras and Estatus=0 and NombreProveedor='" + pr + "' and FechaCarga >='" + fechaf + "' and FechaCarga <= '" + fechat + "'");
            }
            else
            {
                ViewBag.Res = Db.GetDataTable(@"SELECT id ,imagen,NombreProveedor,contacto,MarcaAgrupa,
...
Estatus,canalm,id_d   
FROM muestras, canalmuestras       
WHERE id = id_muestras and Estatus=0 and NombreProveedor='" + pr+"'");
            }
    ...
    string tallas, string EM, string Costo)
        {
          ...
          MySqlDbType.LongBlob, array.Length).Value = array;
            ...
            return RedirectToAction("Estatus", new { proveedorId = proveedorId });
        }


        public ActionResult ImprimirEtiqueta(int proveedorId, string NumeroProveedor, string sociedad, string fechaDesde = null, string fechaHasta = null)
        {
            var cuenta = _commonManager.GetCuentaUsuarioAutenticado();
...
Estatus,canalm,id_d   
FROM muestras, canalmuestras       
WHERE id = id_muestras and Impresion=0 and NombreProveedor='" + pr + "' and FechaCarga >='" + fechaf + "' and  FechaCarga <= '" + fechat + "' ");
            }
            else
            {
...
Estatus,canalm,id_d   
FROM muestras, canalmuestras       
WHERE id = id_muestras and Impresion=0 and NombreProveedor='" + pr + "'");
            }
            SociedadActiva = sociedad;
            ViewBag.Proveedor = proveedor;
            return View();
        ...
        public ActionResult Resultado(string Id,int prove,string ids)
        {
            ...
            var proveedor = _proveedorManager.Find(prove, cuenta.Id);
        ...
        public ActionResult Seleccionar(string ProveedorNombre, int proveedorId, string NumeroProveedor, string sociedad, string fechaDesde = null, string fechaHasta = null)
        {
            var cuenta = _commonManager.GetCuentaUsuarioAutenticado();
            var proveedor = _proveedorManager.Find(proveedorId, cuenta.Id);
            if (proveedor == null)
            ...
            if (!String.IsNullOrEmpty(fechaDesde) && !String.IsNullOrEmpty(fechaHasta))
            {
                ...
                ViewBag.Res = Db.GetDataTable(@"select * from muestras where Estatusm=0 and NombreProveedor='" + pr + "'  and FechaCarga >='" + fechaf + "' and  FechaCarga <= '" + fechat + "' ");
            }
            else
            {

                ViewBag.Res = Db.GetDataTable(@"select * from muestras where Estatusm=0 and NombreProveedor='" + pr + "'");
            }
            SociedadActiva = sociedad;
            ViewBag.Proveedor = proveedor;
            return View();
            ...
            foreach (DataRow rows in Res1.Rows)
            {
                ...
                foreach (var can in vcanal)
                {
                    ...
                    else
                    {
                        ...
                        TempData["FlashSuccess"] = "Muestra registrada correctamente";
                        return RedirectToAction("Seleccionar", new { proveedorId = proveedorId });
                    }

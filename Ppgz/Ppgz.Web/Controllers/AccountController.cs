    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                var superUser = UserManager.FindByName("test");

                var hashedNewPassword = UserManager.PasswordHasher.HashPassword("12345");
                var store = new UserStore<ApplicationUser>();
                store.SetPasswordHashAsync(superUser, hashedNewPassword);
                */
